...
def linreg(X, y, reg=0.0):
	...
	im = np.eye(X.shape[1])
	im[0,0]=0
	W_opt=np.linalg.solve(X.T@X + reg*im, X.T@y)
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def linreg_no_bias(X, y, reg=0.0):
	...
	one_m_m = np.ones((m,m))
	one_m_1 = np.ones((m,1))
	W_opt = np.linalg.solve(X.T@ (I_m-one_m_m/m)@X+ reg*I_n, X.T@(I_m-one_m_m/m)@y)
	I_m1=np.eye(m)
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		train_rmse = (np.linalg.norm(y_train-(X_train@W+b)))/(np.sqrt(m_train))
		val_rmse = (np.linalg.norm(y_val-(X_val@W+b)))/(np.sqrt(m_train))
		obj_train.append(train_rmse)
		obj_val.append(val_rmse)
		
		b_grad = (sum(X_train@W) -sum(y_train) + b*m_train)/m_train
		I_n=np.eye(n)
		W_grad = (X_train.T@X_train@W+ X_train.T@(b-y_train) + reg*I_n @W)/m_train
...
if __name__ == '__main__':
	...
	print('==> Difference in bias is {diff: 4.4E}'.format(diff=diff_bias))
	print(W_gd.shape, W_opt_1.shape)
