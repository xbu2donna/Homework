...
def newton_step(X, y, W, reg=0.0):
	...
	n = X.shape[1]
	u = sigmoid (X@W)
	diag = np.diag(np.squeeze(np.asarray(u*(1-u))))
	W_grad= grad_logreg(X,y, W,reg)
	i_nn = np.eye(n)
	H = X.T@diag@X + reg*i_nn
	d = np.linalg.solve (H, W_grad)
	"*** END YOUR CODE HERE ***"
	return d
...
def get_description(X, y, W):
	...
	count_t1 = 0
	count_r=0 
	predict_y = predict(X,W)

	for i in range(m):
		if y.item(i)==predict_y.item(i):
			count_a+=1

		if predict_y.item(i)==1:
			count_p1+=1
			if predict_y.item(i)==y.item(i):
				count_p+=1
	...
	accuracy = count_a/m
	precision = (count_p)/(count_p1)
	recall = (count_r)/(count_t1)
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	reg_list.sort()
	for reg in reg_list:
		W, nll_list = grad_descent(X_train, y_train,reg=reg, lr=2e-4, print_freq=100)
