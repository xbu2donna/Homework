...
def NLL(X, y, W, reg=0.0):
    ...
    a = y * np.log(u)
    NLL = -a.sum(axis=1).sum() + reg*np.trace(W.T @ W)
    "*** END YOUR CODE HERE ***"
    return NLL
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
    ...
    for reg in lambda_list:
        ...
        "*** YOUR CODE HERE ***"
        W, nll_list = grad_descent(X_train, y_train_OH,reg=reg, lr=2e-5, print_freq=100)
        predict_y = predict(X_test,W)
        accuracy = get_accuracy(predict_y,y_test)
    ...
    i = accu_list.index(a_max)
    reg_opt = lambda_list[i] 
    "*** END YOUR CODE HERE ***"
